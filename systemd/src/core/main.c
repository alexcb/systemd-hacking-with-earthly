...
static int help(void) {
		...
		printf("this is not a real word: alexcb\n");

		printf("This mega-program takes no positional arguments.\n");
...
int main(int argc, char *argv[]) {
		...
		// don't print to stdout here, it breaks things.
        ...
        if (getpid_cached() == 1) {
                ...
                if (detect_container() <= 0) {
                        ...
                        if (!skip_setup) {
                                ...
                                log_info("I am here, no container");
                        ...
                        log_info("I am here, in container");

                        /* For later on, see above... */
                        //log_set_target(LOG_TARGET_JOURNAL); // ACB disable this, force kmsg instead
                        log_info("set log target to kmsg (before)");
						log_set_target(LOG_TARGET_KMSG);
                        log_info("set log target to kmsg (after)");
                ...
                log_set_target(LOG_TARGET_CONSOLE); // ACB force console
                log_open();
                log_info("I am here, in user-mode");
